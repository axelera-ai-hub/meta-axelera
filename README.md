...
This layer provides kernel driver and udev rules for Axelera AI PCIe accelerator devices, enabling seamless integration into Yocto-based Linux distributions.
...
This branch supports **Yocto 3.1 (Dunfell)** only.

For other Yocto versions, check out the corresponding branch:
- `yocto/scarthgap` - Yocto 5.0 (Scarthgap)
- `yocto/kirkstone` - Yocto 4.0 (Kirkstone)
- `yocto/dunfell` - Yocto 3.1 (Dunfell) ← You are here
...
- Dunfell-based Yocto distribution
...
# Clone meta-axelera (dunfell branch)
git clone -b yocto/dunfell https://github.com/axelera-ai/meta-axelera.git
```
...
IMAGE_INSTALL_append = " axelera-driver axelera-udev-rules"
```
...
DMA buffers enable zero-copy data transfer between the CPU and the Axelera AI accelerator, eliminating expensive memory copy operations. This is critical for high-performance inference workloads.
...
KERNEL_CONFIG_FRAGMENTS_append = " file://dmabuf.cfg"
```
